...
const randmps = random_mps(siteinds("Qubit", 10); linkdims=2) 
let
    @time res = mt.invertMPSLiu(randmps, mt.invertSweepLC; eps=0.01)
    #@time result1 = mt.invert(randmps, mt.invertSweepLC; eps=0.01)
    #@time result2 = mt.invert(randmps, mt.invertGlobalSweep; eps=0.01)
end
...
let
    ...
    eps = 1e-2
    psi = random_mps(siteinds("Qubit", N), linkdims = 2)
    mt.invertMPS1(psi, mt.invertSweepLC; eps = eps, pathname = "D:\\Julia\\MyProject\\Data\\randMPS\\")
